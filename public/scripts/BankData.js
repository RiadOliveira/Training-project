function verifyDate () {
    // Verification of expirationDate
        var expString = (exp.value).split('-');

        if(expString != '') {
            
            submitButton.addEventListener('click', verifyDate)

            if(expString[0].length != 4 || Number(expString[0]) < Number(currentYear)) { //Verify if the inputed date is valid
                alert('Data de validade do cartão inválida.')
                submitButton.type = 'button'
            } else if (Number(expString[0]) == Number(currentYear) && Number(expString[1]) < Number(currentMonth)) {
                alert('Data de validade do cartão inválida.')
                submitButton.type = 'button'
            } else {
...
function dataVerify () { //Required to Bank Data
    for(var i in userBankData) {
...
function noBankData () { //Verify the presence of Bank Data and remove required if not
    ...
    if(fields == userBankData.length) { //Activate or desactivate verify date based on the fields filleds by the user
        userBankData.forEach((ind) => {
